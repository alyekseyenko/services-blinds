...
export default function AdminHistoryView({ onSelectOpportunity }: AdminHistoryViewProps) {
  ...
  return (
    <div className="h-full flex flex-col bg-slate-50 overflow-hidden animate-in fade-in duration-500">
      <div className="shrink-0 flex flex-col gap-3 border-b border-slate-100 bg-white p-4 md:flex-row md:items-center md:justify-between lg:gap-4 lg:p-5">
        <div className="min-w-0">
          <h2 className="text-xl font-black uppercase tracking-tighter text-slate-900 italic lg:text-2xl">
            Histórico de Intervenções
          </h2>
          <p className="mt-0.5 text-[10px] font-black uppercase tracking-[0.15em] text-slate-600 lg:text-xs lg:tracking-[0.2em]">
      ...
      <div className="flex-1 overflow-y-auto p-4 md:p-6 lg:p-6">
        ...
        {!loadingHistory && historyOpportunities.length > 0 && (
          ...
          {historyOpportunities.map((opp) => (
            <div
              ...
              className="group flex cursor-pointer flex-col gap-3 rounded-2xl border border-slate-100 bg-white p-4 shadow-sm transition-all hover:border-emerald-200 hover:shadow-md lg:rounded-[1.5rem] lg:p-5"
            >
              <div className="flex min-w-0 items-start gap-3">
                ...
                <div className="min-w-0 flex-1">
                  <div className="mb-1 flex flex-wrap items-center gap-2">
                    <h3 className="truncate text-base font-black uppercase tracking-tighter text-slate-900 italic transition-colors group-hover:text-emerald-600 lg:text-lg">
                      {opp.title}
                    </h3>
                    <span
                      className={`shrink-0 rounded-full px-2 py-0.5 text-[10px] font-black uppercase tracking-widest lg:px-2.5 lg:py-1 lg:text-xs ${
                        opp.status === "Concluído" || opp.status === "CONCLUIDO"
                  ...
                  <div className="flex flex-wrap items-center gap-x-3 gap-y-1 text-[10px] font-bold uppercase tracking-wider text-slate-600 lg:text-xs">
              ...
              <button
                type="button"
                className="min-h-10 w-full rounded-lg bg-slate-50 px-4 py-2 text-[10px] font-black uppercase tracking-widest text-slate-600 transition-all group-hover:bg-slate-950 group-hover:text-white lg:min-h-9 lg:w-auto lg:self-end lg:px-5"
              >
                Ver Detalhes
              </button>
            </div>
        ...
        {!loadingHistory && (historyData?.totalPages ?? 0) > 1 && (
          <div className="mx-auto flex max-w-7xl items-center justify-between border-t border-slate-100 pt-4 lg:col-span-2">
