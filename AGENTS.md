...
- Scripts: `scripts/vps_deploy_and_build.py` (reads `VPS_HOST`, `VPS_PASSWORD`, `DEPLOY_HEALTH_URL` from local env).
- Server path: `/root/app-tecnicos`. Container: `technician-app` on port **3005**.
- `NEXT_PUBLIC_*` vars are baked at Docker build time — patch via `scripts/vps_patch_production_env.py` and rebuild.
