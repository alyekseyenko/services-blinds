...
function formatPhone(phones?: { primaryPhoneNumber?: string; primaryPhoneCallingCode?: string } | null): string | null {
...
function mapTaskNode(node: any): AppTask {
  ...
  const clientPhone =
    formatPhone(personTarget?.phones) ||
    formatPhone(oppTarget?.pointOfContact?.phones) ||
    null;
